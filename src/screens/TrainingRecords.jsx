...
function EquipmentTraining({ students, session }) {
  ...
  async function load() {
    ...
    const [{ data: eq }, { data: rec }] = await Promise.all([
      sb.from('equipment_list').select('*').order('name'),
  ...
  async function addEquipment() {
    ...
    await sb.from('equipment_list').insert({ name: newEquip.name.trim(), description: newEquip.description.trim() || null })
  ...
  async function deleteEquipment(id) {
    ...
    await sb.from('equipment_list').delete().eq('id', id)
...
export default function TrainingRecords() {
  ...
  async function checkExpiry() {
    const { data } = await sb.from('training_equipment').select('*, equipment_list(name), users(name)').lt('expires_at', new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])
        ...
        <div style={{ background: 'var(--warn-light)', border: '1px solid #fcd34d', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: 20, fontSize: 14, color: '#92400e' }}>
          ...
          {expiryAlerts.map((a, i) => (
            <div key={i} style={{ fontSize: 13 }}>
              • <strong>{a.users?.name}</strong> — {a.equipment_list?.name} expires {a.expires_at}
