...
function typeAbbr(type) {
  return {
    aggregate: 'AGG', asphalt_binder: 'AB',
    plant_mix: 'PM', cores: 'CORE', other: 'OTH'
  }[type] || 'MAT'
}
...
function generateBarcodeId(project, material, allMaterials) {
  ...
  // Count how many materials of same type exist (for sequence)
...
function PrintLabel({ material, project, barcodeId }) {
  ...
  return (
    <div id="print-label" style={labelStyle}>
      ...
      <div style={{ width: '100%', borderTop: '1px solid #ddd', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        {material.sampling_date && (
          <div style={{ fontSize: 11, color: '#666', textAlign: 'center' }}>Sampled: {material.sampling_date}</div>
        )}
      </div>
    </div>
  )
...
// ══════════════════════════════════════════════════════════════
// MAIN MATERIAL STORAGE COMPONENT
// ══════════════════════════════════════════════════════════════
export default function MaterialStorage({ project }) {
  ...
  // ── Auto-generate barcode for a material ──────────────────
  async function assignBarcode(material) {
    const generated = generateBarcodeId(project, material, materials)
    const { error } = await sb.from('project_materials')
      .update({ barcode_id: generated, barcode_scanned_at: new Date().toISOString() })
      .eq('id', material.id)
    if (error) { toast('Error assigning barcode.'); return }
    toast(`Barcode assigned: ${generated}`)
    load()
  }
  ...
  async function saveBarcode(material, value) {
    ...
    setSaving(true)
    const { error } = await sb.from('project_materials')
      .update({ barcode_id: value.trim(), barcode_scanned_at: new Date().toISOString() })
      .eq('id', material.id)
    setSaving(false)
  ...
  function printLabel() {
    ...
    win.document.write(`
      <!DOCTYPE html><html><head><title>LabStock Label</title>
      <style>
        @page { size: 4in 4in; margin: 0; }
        body { margin: 0; padding: 0; display: flex; align-items: center; justify-content: center; width: 4in; height: 4in; }
  ...
  if (materials.length === 0) return (
    <div className="empty-state" style={{ padding: 40 }}>
      ...
      <div>No materials yet. Add materials in the Project Materials tab first.</div>
    </div>
  )
  ...
  return (
    <div>
          ...
          {materials.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => setSelectedId(m.id)}
                style={{ padding: '7px 14px', borderRadius: 99, border: `2px solid ${isSelected ? 'var(--accent3)' : 'var(--border)'}`, background: isSelected ? 'var(--accent3-light)' : 'var(--surface)', color: isSelected ? 'var(--accent3)' : 'var(--text2)', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.12s', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20 }}>
            ...
            {selected.barcode_id && editingBarcode !== selected.id ? (
              /* Barcode assigned — show it */
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                  <div style={{ background: 'var(--surface2)', borderRadius: 'var(--radius)', padding: 12, display: 'inline-block' }}>
                    <QRCode value={selected.barcode_id} size={100} />
                  </div>
                  <div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 20, fontWeight: 700, letterSpacing: '0.05em', color: 'var(--text)', marginBottom: 6 }}>{selected.barcode_id}</div>
                    {selected.barcode_scanned_at && (
                      <div style={{ fontSize: 12, color: 'var(--text3)' }}>Assigned {new Date(selected.barcode_scanned_at).toLocaleDateString()}</div>
                    )}
              ...
              <BarcodeEditForm
                material={selected}
                onSave={(val) => saveBarcode(selected, val)}
              ...
              <div>
                <p style={{ fontSize: 14, color: 'var(--text2)', marginBottom: 14 }}>
                  No barcode assigned yet. Auto-generate one or scan/enter manually.
                </p>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button className="btn btn-primary btn-sm" onClick={() => assignBarcode(selected)}>
                    ⚡ Auto-generate
                  </button>
                  <button className="btn btn-sm" onClick={() => setShowScanner(true)}>
                    📷 Scan barcode
                  </button>
                  <button className="btn btn-sm" onClick={() => setEditingBarcode(selected.id)}>
                    ⌨️ Enter manually
                  </button>
                </div>
                <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>
                  Auto-generated format: {generateBarcodeId(project, selected, materials)}
                </div>
              </div>
            )}
          </div>
          ...
          {!selected.storage_confirmed && (
            <button className="btn btn-primary" onClick={() => confirmStorage(selected)}
              style={{ width: '100%', padding: 14, fontSize: 15 }}>
              ✅ Confirm Storage Complete
            </button>
          )}
        </div>
      )}
      ...
      {showScanner && (
        <BarcodeScanner
          onScanned={(val) => { setShowScanner(false); saveBarcode(selected, val) }}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', padding: 28, maxWidth: 480, width: '100%' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 16, textAlign: 'center' }}>
              Make sure your Brother QL-1110NWB is connected and set to 4" tape.
            </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input value={value} onChange={e => setValue(e.target.value.toUpperCase())}
          ...
          autoFocus
        />
        <button className="btn btn-primary btn-sm" onClick={() => onSave(value)} disabled={saving}>
          {saving ? '…' : 'Save'}
        </button>
  ...
  return (
    <div>
      <textarea
        ...
        placeholder="Any special storage instructions, access notes, hazards…"
        style={{ resize: 'vertical', marginBottom: 10 }}
      />
      {dirty && (
        <button className="btn btn-sm btn-primary" onClick={() => { onSave(notes); setDirty(false) }}>Save notes</button>
      )}
    </div>
  )
}
