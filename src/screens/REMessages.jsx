...
export default function REMessages() {
  ...
  async function load() {
    ...
    if (isAdmin) {
      ...
      // Staff sees messages sent to them or by them
      query = query.or(`receiver_id.eq.${session.userId},sender_id.eq.${session.userId}`)
    } else {
      // Students see only their own sent messages and messages addressed to them
      query = query.or(`receiver_id.eq.${session.userId},sender_id.eq.${session.userId}`)
    }
  ...
  const inbox = messages.filter(m => m.receiver_id === session?.userId || (isAdmin && m.receiver_id !== session?.userId) || (!m.receiver_id && (isAdmin || isStaff)))
  ...
  function canEditMsg(m) {
    ...
    if (isStaff && m.sender_id === session?.userId) return true
    return false
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div className="section-title">Research Engineers</div>
          <div style={{ fontSize: 13, color: 'var(--text2)', marginTop: 2 }}>Notes, ideas, questions & issue reports</div>
        </div>
        <button className="btn btn-primary" onClick={() => setTab('compose')}>+ New message</button>
      </div>
      ...
      {tab === 'compose' && (
        <div className="card" style={{ maxWidth: 600 }}>
          ...
          {/* Receiver — students must select, staff/admin optional (broadcast) */}
          <div className="field">
            ...
            <select value={form.receiverId} onChange={e => setForm(f => ({ ...f, receiverId: e.target.value }))}>
              ...
              {staff.map(s => (
                <option key={s.id} value={s.id}>{s.name} ({s.role === 'admin' ? 'Admin' : 'RE/Staff'})</option>
              ))}
            </select>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button className="btn" onClick={() => setTab('inbox')}>Cancel</button>
          </div>
        </div>
      )}
              ...
              <div key={m.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px 20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <div>
                        ...
                        {m.receiver_name && (
                          <span style={{ fontSize: 13, color: 'var(--text3)', marginLeft: 6 }}>→ {m.receiver_name}</span>
                        )}
                        {!m.receiver_id && (
                          <span style={{ marginLeft: 6, fontSize: 11, background: 'var(--accent-light)', color: 'var(--accent)', borderRadius: 3, padding: '1px 6px' }}>Broadcast</span>
                        )}
                      </div>
                    </div>
                  ...
                  <div style={{ display: 'flex', align: 'center', gap: 8, alignItems: 'center' }}>
                    <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)', whiteSpace: 'nowrap' }}>
                      ...
                      {m.edited && <span style={{ marginLeft: 6, color: 'var(--text3)', fontStyle: 'italic' }}>(edited)</span>}
                    </div>
                    {canEditMsg(m) && (
                      <button className="btn btn-sm" style={{ padding: '3px 8px', fontSize: 11 }}
                        onClick={() => setEditMsg({ ...m })}>Edit</button>
                    )}
                    {canDeleteMsg(m) && (
                      <button className="btn btn-sm btn-danger" style={{ padding: '3px 8px', fontSize: 11 }}
                        onClick={() => setDeleteConfirm(m.id)}>✕</button>
                    )}
                  </div>
                </div>

                {/* Body */}
                <div style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.7, whiteSpace: 'pre-wrap', marginBottom: m.file_url ? 10 : 0 }}>
                  {m.body}
                </div>
                ...
                {m.file_url && (
                  <a href={m.file_url} target="_blank" rel="noopener"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--accent)', marginTop: 8, textDecoration: 'none', background: 'var(--accent-light)', borderRadius: 6, padding: '4px 10px' }}>
      ...
      {editMsg && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', padding: 24, maxWidth: 520, width: '100%', border: '1px solid var(--border)' }}>
            ...
            <div className="field">
              <label>Subject</label>
              <input value={editMsg.subject || ''} onChange={e => setEditMsg(m => ({ ...m, subject: e.target.value }))} />
            </div>
            <div className="field">
              <label>Message</label>
              <textarea rows={5} value={editMsg.body} onChange={e => setEditMsg(m => ({ ...m, body: e.target.value }))} style={{ resize: 'vertical' }} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
