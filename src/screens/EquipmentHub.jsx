...
function EquipmentInfo({ equipment, session }) {
  ...
  const [sopStep, setSopStep] = useState(null) // step-by-step view index
  ...
  async function load() {
    ...
    const [{ data: det }, { data: vid }, { data: s }] = await Promise.all([
      ...
      sb.from('equipment_sop').select('*').eq('equipment_id', equipment.id).single(),
    ])
    setDetails(det || null)
    setVideos(vid || [])
    setSop(s || null)
  ...
  function addStep() {
    ...
    setSopForm(f => ({ ...f, steps: [...f.steps, { text: newStep.trim(), order: f.steps.length + 1 }] }))
    setNewStep('')
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          {canEdit(session) && <button className="btn btn-sm btn-primary" onClick={() => { setSopForm(sop ? { title: sop.title||'', pdf_url: sop.pdf_url||'', steps: sop.steps||[] } : { title: '', pdf_url: '', steps: [] }); setShowSopForm(true) }}>
...
function StandardsTab({ equipment, session }) {
  ...
  async function deleteStandard(id) {
    ...
    await sb.from('equipment_standards').delete().eq('id', id)
    load(); toast('Standard removed.')
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 20 }}>
        {STD_TYPES.map(t => (
          <button key={t} onClick={() => { setActiveType(t); setShowForm(false) }}
            ...
            {t}
            {standards.filter(s => s.standard_type === t).length > 0 && (
              <span style={{ marginLeft: 6, fontSize: 11, background: 'var(--accent-light)', color: 'var(--accent)', borderRadius: 99, padding: '1px 7px' }}>
                {standards.filter(s => s.standard_type === t).length}
              </span>
            )}
          </button>
        ))}
      ...
      {canEdit(session) && (
        <div style={{ marginBottom: 16 }}>
          <button className="btn btn-sm btn-primary" onClick={() => { setForm(f => ({ ...f, standard_type: activeType })); setShowForm(true) }}>+ Add {activeType} standard</button>
        </div>
      )}
      ...
      {showForm && (
        <div style={{ background: 'var(--surface2)', borderRadius: 'var(--radius-lg)', padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 8 }}>Attach standard (upload file OR enter link):</div>
          <div className="grid-2">
          ...
          <table>
            ...
            <tbody>
              {filtered.map(std => (
                <tr key={std.id}>
                  <td>
                    {std.file_url || std.link_url ? (
                      <a href={std.file_url || std.link_url} target="_blank" rel="noopener"
                        style={{ fontWeight: 700, color: 'var(--accent)', textDecoration: 'none', fontFamily: 'var(--mono)', fontSize: 14 }}>
                        {std.standard_number}
                      </a>
                    ) : (
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 8 }}>
                      {std.file_url && <a href={std.file_url} target="_blank" rel="noopener" className="btn btn-sm" style={{ fontSize: 11, padding: '3px 8px' }}>📄 Download</a>}
                      {std.link_url && <a href={std.link_url} target="_blank" rel="noopener" className="btn btn-sm" style={{ fontSize: 11, padding: '3px 8px' }}>🔗 Link</a>}
...
export default function EquipmentHub() {
  ...
  return (
    <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', minHeight: 500 }}>

      {/* ── Left panel: equipment list ── */}
      <div style={{ width: 260, flexShrink: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            filtered.map(e => (
              <div key={e.id} onClick={() => { setSelected(e); setSubTab('info') }}
                ...
                <div style={{ fontWeight: selected?.id === e.id ? 600 : 500, fontSize: 13, color: selected?.id === e.id ? 'var(--accent)' : 'var(--text)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {!selected ? (
          <div className="empty-state" style={{ marginTop: 60 }}>
            ...
            <div>Select equipment from the list to view details</div>
          </div>
        ) : (
          <div>
            {/* Header */}
            ...
            {/* Sub-tabs */}
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 20 }}>
              {[{ key: 'info', label: '📋 Equipment Info' }, { key: 'standards', label: '📑 Standards' }].map(t => (
