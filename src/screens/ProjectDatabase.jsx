...
async function extractPdfText(file) {
  try {
    ...
    return text.trim().slice(0, 50000) // cap at 50k chars
  } catch (e) {
    return ''
  }
}

// ── AI search using Claude API ────────────────────────────────
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `You are a document search assistant. Given a search query and a list of documents, return the indices of documents that are relevant to the query. Consider file names, descriptions, and extracted text content.
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', marginBottom: 10, background: 'var(--surface)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          <a href={file.file_url} target="_blank" rel="noopener" className="btn btn-sm" onClick={e => e.stopPropagation()}>
      ...
      {/* Expanded: show extracted text preview or image */}
      {expanded && (
        <div style={{ borderTop: '1px solid var(--border)', padding: '14px 18px', background: 'var(--surface2)' }}>
          {isImage && (
            <img src={file.file_url} style={{ maxWidth: '100%', maxHeight: 300, borderRadius: 'var(--radius)', marginBottom: file.extracted_text ? 12 : 0 }} />
          )}
          {file.extracted_text ? (
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Extracted text preview</div>
              <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, maxHeight: 150, overflow: 'hidden', position: 'relative' }}>
                {file.extracted_text.slice(0, 600)}{file.extracted_text.length > 600 ? '…' : ''}
              </div>
            </div>
          ) : (
...
export default function ProjectDatabase({ project }) {
  ...
  async function uploadFiles() {
    ...
    for (const file of pendingFiles) {
      try {
        ...
        if (file.type.includes('pdf')) {
          ...
          extracted_text = await extractPdfText(file)
        }
    ...
    await load()
    toast(`${uploaded} file${uploaded !== 1 ? 's' : ''} uploaded successfully.`)
  }
  ...
  async function deleteFile(file) {
    ...
    if (pathParts[1]) {
      await sb.storage.from('project-files').remove([pathParts[1]])
    }
  ...
  function exportIndex() {
    const data = [
      ['LabStock — Project File Index'],
      ['Project:', project.name],
      ['Exported:', new Date().toLocaleString()],
      [],
      ['File Name', 'Type', 'Size', 'Description', 'Uploaded By', 'Date', 'URL']
    ]
    files.forEach(f => data.push([
      f.file_name, f.file_type, formatSize(f.file_size),
      f.description || '', f.uploaded_by || '',
      new Date(f.created_at).toLocaleDateString(), f.file_url
    ]))
    const ws = XLSX.utils.aoa_to_sheet(data)
    ws['!cols'] = [{wch:40},{wch:20},{wch:10},{wch:40},{wch:16},{wch:14},{wch:60}]
  ...
  return (
    <div>
      ...
      {files.length > 0 && (
        <div style={{ marginBottom: 20, position: 'relative' }}>
          <input
            ...
            placeholder="🔍 AI search — type anything to find it across all files…"
          ...
          {query && !searching && (
            <button onClick={() => { setQuery(''); setFiltered(files) }}
              style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: 16 }}>✕</button>
          )}
          {query && !searching && (
      ...
      {/* ── Drop zone overlay (when files exist) ── */}
      {files.length > 0 && (
        <div
          onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor = 'var(--accent3)' }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', padding: 28, maxWidth: 480, width: '100%', border: '1px solid var(--border)' }}>
            ...
            <div className="field">
              ...
              <textarea rows={2} value={uploadForm.description}
                ...
                placeholder="e.g. Lab test results, April 2026…"
                style={{ resize: 'vertical' }}
              />
            </div>

            {pendingFiles.some(f => f.type.includes('pdf')) && (
              <div style={{ background: 'var(--accent-light)', borderRadius: 'var(--radius)', padding: '10px 14px', fontSize: 13, color: 'var(--accent)', marginBottom: 16 }}>
                📕 PDF detected — text will be auto-extracted for AI search
              </div>
            )}

            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn btn-purple" onClick={uploadFiles} disabled={uploading} style={{ flex: 1 }}>
