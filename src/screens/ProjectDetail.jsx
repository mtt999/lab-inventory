...
export default function ProjectDetail() {
  ...
  async function addSupply() {
    ...
    setSupplyForm({ name:'', quantity:1, unit:'pcs', notes:'' })
    load()
    toast('Item added.')
  }
  ...
  async function deleteSupply(id) {
    ...
    await sb.from('project_supplies').delete().eq('id', id)
    load()
    toast('Item removed.')
  }
    ...
    <div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: project.notes ? 16 : 0 }}>
          {[
            ...
            ['SAMPLING DATE', project.sampling_date || '—'],
            ['STORAGE DATE', project.storage_date || '—'],
            ['CREATED', new Date(project.created_at).toLocaleDateString()],
          ].map(([label, val]) => (
            <div key={label}>
              <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)', marginBottom: 4 }}>{label}</div>
          ...
          supplies.map(s => (
            <div key={s.id} className="supply-row">
              <div>
                ...
                {s.notes && <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>{s.notes}</div>}
              </div>
      ...
      {showEditModal && (
        <Modal onClose={() => setShowEditModal(false)}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="btn btn-purple" onClick={saveEdit}>Save</button>
      ...
      {showAddSupply && (
        <Modal onClose={() => setShowAddSupply(false)}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="btn btn-purple" onClick={addSupply}>Add item</button>
            <button className="btn" onClick={() => setShowAddSupply(false)}>Cancel</button>
          </div>
        </Modal>
      )}
    </div>
